...
if ~isfield(display,'center')
    display.center = display.resolution/(2);
elseif isfield(display,'center') %added this to fit my screen
    % for waisman setup, display.center should stay the same
    %display.center = [650,400];  %added this to fit my screen
end
...
if ~isfield(display.fixation,'mask')
    display.fixation.mask = 0;  % In degrees, originally 2 but set equal to zero to get rid of mask
end
...
for i=1:3
    rect{i}= [-sz(i)+display.center(1),-sz(i)+display.center(2),sz(i)+display.center(1),sz(i)+display.center(2)];
end
