function [filename] = gen_fmtMatrixByCond_attentionComp(dataPath,indBase,indShift,dataValsStr,bMels,bFilt,bSaveCheck)
            ...
            if isfield(expt,'shifts')
                if bMels
                    if isfield(expt.shifts,'mels')
                        shiftvec = expt.shifts.mels{indShift(c).shiftind+1}; % shiftvec = expt.shifts.mels{indShift(c).shiftind}
                    else
