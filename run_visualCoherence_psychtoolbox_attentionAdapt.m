function [data,exptThresh] = run_visualCoherence_psychtoolbox_attentionAdapt(snum, exptThresh, bTestMode)
...
if nargin < 2 || isempty(exptThresh)
    ...
    exptThresh.dataPath = fullfile(get_acoustSavePath('attentionAdapt',exptThresh.snum), 'coherence');
...
display.screenNum = 2;          % CWN changed to 2 for Waisman setup; 0 is default
...
display.bDebug = 0; % turn to 1 if you need to debug PsychToolBox issues
...
dots.apertureSize = [7, 7];   % size of rectangular aperture [w,h] in degrees, [5, 5]
dots.speed = 3;
dots.lifetime = 12;
